 ...
 *
 * Part of the grexp/gh-issue-jira-sync public GitHub Action.
...
for (const key of ['GITHUB_TOKEN', 'GH_REPO', 'JIRA_BASE_URL', 'JIRA_USER_EMAIL', 'JIRA_API_TOKEN', 'JIRA_PROJECT_KEY']) {
...
function buildDescription(issue, meta) {
  ...
  const content = [
    {
      type: 'paragraph',
      content: [
        { type: 'text', text: 'Auto-migrated from GitHub Issues by ', marks: [{ type: 'em' }] },
        { type: 'text', text: 'gh-issue-jira-sync', marks: [{ type: 'em' }, { type: 'strong' }] },
  ...
  if (meta.codeBlock) {
    ...
    content.push(
      { type: 'heading', attrs: { level: 3 }, content: [{ type: 'text', text: 'Code Context' }] },
...
async function ensureLabel(name, color = '0075ca', description = '') {
  try {
    await octokit.issues.getLabel({ owner: GH_OWNER, repo: GH_REPO_NAME, name });
  } catch {
    await octokit.issues.createLabel({ owner: GH_OWNER, repo: GH_REPO_NAME, name, color, description });
  }
}
...
async function applyJiraLabel(issueNumber, jiraKey) {
  ...
  if (ENV.DRY_RUN) { log('  [DRY RUN] Skipping label creation'); return; }
  await ensureLabel(name, '0075ca', `Synced to Jira as ${jiraKey}`);
  await octokit.issues.addLabels({ owner: GH_OWNER, repo: GH_REPO_NAME, issue_number: issueNumber, labels: [name] });
}
...
async function closeIssue(issueNumber, jiraKey) {
  ...
  if (ENV.DRY_RUN) { log('  [DRY RUN] Skipping close'); return; }

  await octokit.issues.createComment({ owner: GH_OWNER, repo: GH_REPO_NAME, issue_number: issueNumber, body });
  await octokit.issues.update({ owner: GH_OWNER, repo: GH_REPO_NAME, issue_number: issueNumber, state: 'closed', state_reason: 'not_planned' });
}
...
async function syncIssue(issueNumber) {
  ...
  const { data: issue } = await octokit.issues.get({ owner: GH_OWNER, repo: GH_REPO_NAME, issue_number: issueNumber });

  const existingKey = getJiraKeyFromLabels(issue.labels);
  if (existingKey) {
    log(`  ⏭  Already synced as ${existingKey} — skipping`);
    return { status: 'skipped', reason: 'already_synced', jiraKey: existingKey };
  }
...
async function bulkSync() {
  ...
  for (let page = 1; ; page++) {
    const { data: issues } = await octokit.issues.listForRepo({
      owner: GH_OWNER, repo: GH_REPO_NAME,
